...
for i, pins in enumerate(tracks) :
	...
	for n in pins :
		...
		if n in top_pins :
			init = top_pins.index(n)
			end = len(top_pins)-rtop.index(n)-1

		if n in bottom_pins :
			foo = bottom_pins.index(n)
			init = foo if init > foo else init
			foo = len(bottom_pins)-rbottom.index(n)-1
